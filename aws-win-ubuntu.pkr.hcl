...
build {
  ...
  provisioner "shell-local" {
    ...
    inline = [
      ...
      ". /opt/pipx/venvs/ansible-core/bin/activate && /opt/pipx/venvs/ansible-core/bin/python -m pip install pywinrm && ansible-playbook -i '1.2.3.4,' ./win_playbook.yml -c winrm --extra-vars 'ansible_port=5985 ansible_user=user ansible_password=password win_password=password'",
    ]
  }
